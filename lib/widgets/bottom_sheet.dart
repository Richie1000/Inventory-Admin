  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedBuilder(
        animation:
            widget.route.animation ?? AlwaysStoppedAnimation<double>(0.0),
        builder: (BuildContext context, Widget? child) {
          // Adjust the type of child parameter to be nullable
...
class _ModalBottomSheetRoute<T> extends PopupRoute<T> {
  ...
  AnimationController createAnimationController() {
    ...
    // Check if the navigator's overlay is not null before using it
    final overlay = navigator!.overlay;
    assert(overlay != null); // Ensure overlay is not null
